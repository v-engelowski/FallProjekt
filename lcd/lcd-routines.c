...
{
	LCD_CONTROL_PORT |= (1<<LCD_EN);     // Enable auf 1 setzen
	_delay_us( LCD_ENABLE_US );  // kurze Pause
	LCD_CONTROL_PORT &= ~(1<<LCD_EN);    // Enable auf 0 setzen
}
...
static void lcd_out( uint8_t data )
{
	...
	LCD_DATA_PORT &= ~(0xF0>>(4-LCD_DB));    // Maske l�schen
...
void lcd_init( void )
{
	...
	LCD_DDR_CONTROL  |= (1<<LCD_RS) | (1<<LCD_EN);               // R/S Leitung(1<<LCD_EN);
	LCD_DDR_DATEN |= (0x0F << LCD_DB);
	// initial alle Ausg�nge auf Null
	LCD_DATA_PORT &= ~pins;
	LCD_CONTROL_PORT &= ~pins;
...
{
	LCD_CONTROL_PORT |= (1<<LCD_RS);    // RS auf 1 setzen
...
{
	LCD_CONTROL_PORT &= ~(1<<LCD_RS);    // RS auf 0 setzen
